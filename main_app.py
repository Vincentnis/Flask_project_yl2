...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        if file.filename:  # если файл добавили
            ...
            print(avatar_url)
        resp = put(f"http://localhost:8000/api/users/{current_user.id}",  # обновляем данные пользователя
...
def messaging(receiver_id):
    if current_user.id == receiver_id:  # проверяем то, что мы отправляем сообщение не себе
        abort(403)
